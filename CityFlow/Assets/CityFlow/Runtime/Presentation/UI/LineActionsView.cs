...
namespace CityFlow.Presentation.UI
{
    ...
    public sealed class LineActionsView : MonoBehaviour
    {
        ...
        { network=state; session=connection; overview=input; controller=cameraController; document=GetComponent<UIDocument>(); if(isActiveAndEnabled) Bind(); }
        private void OnEnable()=>Bind();
        private void Bind()
        {
            Unbind(); if(document==null||network==null||overview==null||controller==null) return;
            root=document.rootVisualElement;
            Button("line-edit",controller.EditSelectedLine);
            Button("line-delete",()=>{ if(overview.Selected.LineId is int id) network.RequestDeletion(id); });
            Button("line-cancel",()=>{ if(overview.Selected.LineId is int id) network.CancelPending(id); });
        }
        private void Button(string name,Action action)
        { if(root==null) return; var b=root.Q<Button>(name); b.clicked+=action; bindings.Add((b,action)); }
        private void LateUpdate()
        {
            ...
            root.Q<Button>("line-edit").SetEnabled(running); root.Q<Button>("line-delete").SetEnabled(running);
            root.Q<Button>("line-cancel").SetEnabled(!running);
            string state=line.Status==LineStatus.DeletePending ? "DELETE PENDING" : line.Status==LineStatus.RouteChangePending ? "ROUTE CHANGE PENDING" : "RUNNING";
        ...
        private void Unbind()
        { foreach(var b in bindings) b.button.clicked-=b.action; bindings.Clear(); root=null; }
