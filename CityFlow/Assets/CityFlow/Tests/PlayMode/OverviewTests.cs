...
namespace CityFlow.Tests.PlayMode
{
    public sealed class OverviewTests
    {
        ...
        [UnityTest] public IEnumerator HoverShowsNodeAndLineDetailsWithoutPinningSelection()
        {
            ...
            Assert.That(HudAssertions.TooltipText(tooltipRoot), Does.Contain("S1").And.Contain("BUFFER").And.Contain("OUT"));
            ...
            yield return null;
            Assert.That(HudAssertions.TooltipText(tooltipRoot), Does.Contain("S1 → RED").And.Contain("THROUGHPUT").And.Contain("STOPPED"));
